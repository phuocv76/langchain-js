...
- `apps/web` — Next.js + CopilotKit + authenticated runtime proxy + ChatGPT-style UI.
...
flowchart LR
  subgraph web [apps/web: Next.js]
    ...
    Proxy["Authenticated /api/copilotkit proxy"]
  end
  ...
  Provider -->|same-origin HTTP stream| Proxy
  Proxy -->|verified user identity| Runtime
...
and proxies to it. The browser uses the Next.js `/api/copilotkit` route, which
verifies the Firebase session before forwarding a trusted user identity.
...
# production must also share COPILOT_RUNTIME_SECRET between both app env files
...
Hooks are in place for MCP, human-in-the-loop (`interrupt()`), vector DBs, RAG,
memory, and LangSmith observability. Regression tests cover auth boundaries,
runtime protection, environment modes, and REST context conversion.
